<template>
    <v-container fluid class="book-a-table" data-aos="fade-up">
        ...
        <v-row>
            <v-col cols="12" md="4">
                <v-text-field v-model="name" label="訂位人姓名" placeholder="訂位人姓名" autocomplete="off" outlined
                    :rules="nameRules" required :readonly="true" hint="只能更改用餐人數及備註"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
                <v-text-field v-model="email" label="訂位人Email" placeholder="訂位人Email" autocomplete="off" outlined
                    :rules="emailRules" required :readonly="true" hint="只能更改用餐人數及備註"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
                <v-text-field v-model="phone" label="訂位人手機號碼" placeholder="訂位人手機號碼" autocomplete="off" outlined
                    :rules="phoneRules" required :readonly="true" hint="只能更改用餐人數及備註"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
                <v-text-field v-model="date" label="用餐日期" placeholder="用餐日期" autocomplete="off" outlined required
                    :readonly="true" hint="只能更改用餐人數及備註"></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
                <v-autocomplete v-model="adults" label="請選擇用餐人數" placeholder="請選擇用餐人數" :items="adultsOptions"
                    item-text="title" item-value="value" outlined required variant="outlined"></v-autocomplete>
            </v-col>
            <v-col cols="12" md="4">
                <v-autocomplete v-model="kids" label="請選擇小孩數" placeholder="請選擇小孩數" :items="childOptions"
                    item-text="title" item-value="value" outlined required variant="outlined"></v-autocomplete>
            </v-col>
        </v-row>
        <v-row>
            <v-col cols="12">
                <v-textarea v-model="messageData" label="其他備註" placeholder="其他備註" rows="5" outlined
                    variant="outlined"></v-textarea>
            </v-col>
        </v-row>
        <v-row>
            <v-col cols="12" class="text-center">
                <v-btn color="#FFE5B4" @click="putOrder">確認更改</v-btn>
            </v-col>
        </v-row>
    </v-container>
</template>
...
import { getOrderByRandomApi, putOrderApi } from '@/apis/seat.js';
// API 資料傳遞
...
//     const res = await getOrderByIdApi(route.query.id);
//     console.log(res);
// }

const target = ref({});
const getOrderByRandom = async () => {
    const res = await getOrderByRandomApi(route.query.random);
    target.value = res;
    console.log(res)
}
...
const putOrder = async () => {
    ...
    form.append('id', target.value.id);
    form.append('random', target.value.random);
    form.append('adult', adults.value);
    form.append('child', kids.value);
    form.append('message', messageData.value);
    const res = await putOrderApi(form);
...
onMounted(async () => {
    ...
    email.value = target.value.email;
    // 提取 phone 参数
    // const phoneParam = route.query.phone;
    phone.value = target.value.tel;
    // 提取 adult 参数
    // const adultParam = parseInt(route.query.adult);
    adults.value = isNaN(target.value.adult) ? 2 : target.value.adult;
    // 提取 child 参数
    // const childParam = parseInt(route.query.child);
    kids.value = isNaN(target.value.child) ? 0 : target.value.child;
    // 提取 message 参数
    // const messageParam = route.query.message;
    messageData.value = target.value.message;
});
</script>
